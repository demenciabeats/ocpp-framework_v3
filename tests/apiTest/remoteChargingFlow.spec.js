...
test.describe('Flujo de carga remota', () => {
  test('Carga remota con Start/Stop Transaction', async ({ request }) => {
    // Crear y conectar cliente OCPP
    const wsUrl = process.env.WS_URL;
    const chargePointId = process.env.CHARGE_POINT_ID;
    
    if (!wsUrl || !chargePointId) {
      throw new Error('Variables de entorno WS_URL y CHARGE_POINT_ID requeridas');
    }
